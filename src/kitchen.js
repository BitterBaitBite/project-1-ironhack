class Kitchen extends Entity {
	...
	updateKitchen(framesCounter) {
		...
		if (this.time >= 5 && this.cooking) {
			this.food = new Food(this.context, this.position.x + this.size.w / 2, this.position.y, 104 / 2, 68 / 2, 'tiles/dish-empty.png', this.typeOfFood);
			this.currentInteractions--;
			this.cooking = false;
		}
	}
	...
	draw() {
		...
		if (!this.cooking && this.food == undefined) {
			this.context.drawImage(this.imgEmpty, this.position.x, this.position.y, 88 / 2, 88 / 2);
			this.context.drawImage(this.imgKnife, this.position.x + this.size.w / 2, this.position.y, 88 / 2, 88 / 2);
		}
		if (this.food !== undefined) this.food.draw();

		if (this.cooking) {
			this.context.drawImage(this.imgCooking, this.position.x, this.position.y, 88 / 2, 88 / 2);
		}
